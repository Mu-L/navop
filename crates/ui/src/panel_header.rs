use crate::{ActiveTheme as _, LayoutSizeTokens, StyledExt as _, h_flex, tooltip::Tooltip};
use gpui::{
...
impl PanelHeaderVariant {
    fn height(self, layout: &LayoutSizeTokens) -> Pixels {
        match self {
...
mod tests {
    ...
    fn variants_resolve_to_geometry_roles() {
        ...
        assert_eq!(
            PanelHeaderVariant::Panel.height(&layout),
            layout.panel_header
        );
        assert_eq!(
            PanelHeaderVariant::Embedded.height(&layout),
            layout.embedded_panel_header
        );
        assert_eq!(
            PanelHeaderVariant::Sidebar.height(&layout),
            layout.embedded_panel_header
        );
        assert_eq!(
            PanelHeaderVariant::Dock.height(&layout),
            layout.dock_panel_header
        );
        assert_eq!(
            PanelHeaderVariant::Toolbar.height(&layout),
            layout.command_bar
        );
    }
